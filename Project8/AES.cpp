#include <stdint.h>

#include <stdint.h>

// S-Box lookup table
static const uint8_t aes_sbox[256] = {
    /* S-Box values */
    , 0X0c, 0xBA, 0x78, 0x25, 0x2E, 0x1C, 0xA6, 0xB4, 0xC6, 0xE8, 0xDD, 0x74, 0x1F,
    0x4B, 0xBD, 0x8B, 0x8A, 0x70, 0x3E, 0xB5, 0x66, 0x48, 0x03, 0xF6, 0x0E, 0x61, 0x35, 0x57, 0xB9,
    0x86, 0xC1, 0x1D, 0x9E, 0xE1, 0xF8, 0x98, 0x11, 0x69, 0xD9, 0x8E, 0x94, 0x9B, 0x1E, 0x87, 0xE9,
    0xCE, 0x55, 0x28, 0xDF, 0x8C, 0xA1, 0x89, 0x0D, 0xBF, 0xE6, 0x42, 0x68, 0x41, 0x99, 0x2D, 0x0F 

};

// Perform AES S-Box substitution
void aes_sub_bytes(uint8_t* state) {
    for (int i = 0; i < 16; i++) {
        state[i] = aes_sbox[state[i]];
    }
}

// Perform AES Shift Rows operation
void aes_shift_rows(uint8_t* state) {
    uint8_t temp;

    // Shift second row
    temp = state[1];
    state[1] = state[5];
    state[5] = state[9];
    state[9] = state[13];
    state[13] = temp;

    // Shift third row
    temp = state[2];
    state[2] = state[10];
    state[10] = temp;
    temp = state[6];
    state[6] = state[14];
    state[14] = temp;

    // Shift fourth row
    temp = state[15];
    state[15] = state[11];
    state[11] = state[7];
    state[7] = state[3];
    state[3] = temp;
}

// Perform AES Mix Columns operation
void aes_mix_columns(uint8_t* state) {
    for (int i = 0; i < 4; i++) {
        uint8_t s0 = state[i];
        uint8_t s1 = state[i + 4];
        uint8_t s2 = state[i + 8];
        uint8_t s3 = state[i + 12];

        state[i] = aes_mul(0x02, s0) ^ aes_mul(0x03, s1) ^ s2 ^ s3;
        state[i + 4] = s0 ^ aes_mul(0x02, s1) ^ aes_mul(0x03, s2) ^ s3;
        state[i + 8] = s0 ^ s1 ^ aes_mul(0x02, s2) ^ aes_mul(0x03, s3);
        state[i + 12] = aes_mul(0x03, s0) ^ s1 ^ s2 ^ aes_mul(0x02, s3);
    }
}

// Perform AES Add Round Key operation
void aes_add_round_key(uint8_t* state, const uint8_t* round_key) {
    for (int i = 0; i < 16; i++) {
        state[i] ^= round_key[i];
    }
}

// Perform AES multiplication in Galois field
uint8_t aes_mul(uint8_t a, uint8_t b) {
    uint8_t result = 0;
    uint8_t mask = 0x01;

    for (int i = 0; i < 8; i++) {
        if (b & mask) {
            result ^= a;
        }
        mask <<= 1;

        // Check if the high bit is set
        if (a & 0x80) {
            a <<= 1;
            a ^= 0x1B; // XOR with Rijndael's irreducible polynomial
        }
        else {
            a <<= 1;
        }
    }

    return result;
}



void aes_encrypt(uint8_t* output, const uint8_t* input, const uint8_t* round_keys) {
    uint8_t state[16];
    uint8_t round_keys_copy[16];

    // Load input block
    for (int i = 0; i < 16; i++) {
        state[i] = input[i];
    }

    // Copy round keys
    for (int i = 0; i < 16; i++) {
        round_keys_copy[i] = round_keys[i];
    }

    // Perform AES encryption rounds
    for (int round = 0; round < 10; round++) {
        for (int i = 0; i < 16; i++) {
            state[i] = aes_sbox[state[i]];
        }
        aes_shift_rows(state);
        if (round != 9) {
            aes_mix_columns(state);
        }
        aes_add_round_key(state, round_keys_copy + round * 16);
    }

    // Store encrypted block
    for (int i = 0; i < 16; i++) {
        output[i] = state[i];
    }
}
