...
def ext_gcd(a, b): #扩展欧几里得算法 ，即 a mod b 的逆元,返回的第一个值为结果
    if b == 0:
        ...
        return x,y,gcd
def sameAdd(x1,y1):#计算椭圆曲线上2*(x1,y1)
    ...
    return x3,y3
def NsameAdd(x1,y1,x_G,y_G):#返回(x3,y3)=(x1,y1)+(x_G,y_G)
    lamda1 = (y_G - y1) % p
    ...
    y3 = ((lamda * (x1 - x3)%p) % p - y1) % p
    return x3,y3
def multiply(x1,y1,k,x_G,y_G):#调用函数(2)(3)，使用类似快速模指数算法，计算(x1,y1)=k(x_G,y_G)
    if k == 2:
    ...
    if k % 2 == 1:
        ...
        return x1,y1
def Point_bit(x1,y1):#选用未压缩格式，将(x1,y1)化为比特串
    X1 = bin(x1)[2:].zfill(256)
    Y1 = bin(y1)[2:].zfill(256)
    return '00000100' + X1 + Y1  #选用未压缩格式
def SHA256(str):#利用hashlib.sha256()将str加密
...
def KDF(x,klen):#调用SHA256函数进行密钥扩展。
    ct = 0x00000001
    K = ""
...
def xor(bit_M,t):#进行逐比特异或
    ...
    for i in range(len(bit_M)):
        ...
        if add == 1:
            ...
            res = res + "0"
    return res
def bit_Point(hexC1):#将16进制字符串转为点坐标
    x = int(hexC1[-128:-64], 16)
    y = int(hexC1[-64:], 16)
    if int(hexC1[:2], 16) == 4:
        return x, y
def Point_C1_in(Point_C1):#判断这个点是否在方程上
    x,y = Point_C1
